...
pcl::PointCloud< PointT >::Ptr generatePointCloud( int index_i)
{
	  ...
	  //cout<<"T:"<<T.matrix()<<endl;
		...
		passthrough.setFilterLimits(-55.0,55.0);//设置直通滤波器操作范围 KITTI 参数 -1.0,15.0
		...
		passthrough.setFilterLimits(-55.0,55.0);//设置直通滤波器操作范围 KITTI 参数 -6.0,6.0
		passthrough.setFilterLimitsNegative(false);//true表示保留范围内，false表示保留范围外
		passthrough.filter(*tmp);//执行滤波，过滤结果保存在 cloud_after_PassThrough
       ...
       std::cout << "直通滤波后点云数据点数：" << cloud_after_PassThrough->points.size() << std::endl;
		...
		cout<<YELLOW<<"find bug:"<< cloud_voxel_tem->points.size() <<WHITE<<endl;
		...
		// pcl::transformPointCloud( *cloud_after_PassThrough, *cloud1, T.matrix());
...
// 		pub_global_pointcloud.publish(output);
		 ...
		 cout<<"数据大小：="<<sizeof(globalMap)<<endl;
...
void callback_pointcloud(const sensor_msgs::PointCloud2::ConstPtr msgPointCloud,
								const geometry_msgs::PoseStamped::ConstPtr tcw )
 {
   ...
   //TODO 旋转变化
//    tf::Vector3 v_1(0,1,0);
//    tf::Quaternion q1(Tcw.pose.orientation.w,Tcw.pose.orientation.x,Tcw.pose.orientation.y,Tcw.pose.orientation.z);
//    q1.setRotation(v_1, M_PI/4);

	// Eigen::Quaterniond q =Eigen::Quaterniond(q1[3],q1[0],q1[1],q1[2] );
 ...
 void shutdown()
{
		   ...
		   {	
                               // cout<<"gogogo"<<endl;
				string save_path = "/home/zeen/my_lib/resultPointCloudFile_KITTI.pcd";
